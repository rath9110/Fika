...
const PeopleList: React.FC<Props> = ({ contacts, onSelect }) => {
    ...
    if (contacts.length === 0) {
        return (
            <div className="py-8 md:py-12 text-center animate-in">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-fika-100/30 rounded-full flex items-center justify-center mx-auto mb-6 md:mb-8 text-fika-200">
                    <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M22 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>
                </div>
    ...
    return (
        <div className="animate-in font-sans">
            <h2 className="text-[11px] font-black text-fika-300 uppercase tracking-[0.25em] px-2 sticky top-0 bg-cream z-20 py-6 mb-2">Your People</h2>
                ...
                {sortedByDue.map((c) => {
                    ...
                    return (
                        <button
                            key={c.id}
                            onClick={() => onSelect(c)}
                            className={`w-full flex justify-between items-center p-8 text-left transition-all duration-300 group relative border-l-[6px] border-transparent
                hover:bg-fika-50/50 first:rounded-t-[3rem] last:rounded-b-[3rem]
                ${isBday ? 'hover:border-orange-400' : 'hover:border-fika-900'}`}
                        >
